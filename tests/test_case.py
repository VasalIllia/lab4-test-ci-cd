...
class TestProduct(unittest.TestCase):
    ...
    def test_add_available_amount(self):
        ...
        self.assertEqual(self.cart.contains_product(self.product), True,
                         'Продукт має успішно додатися до корзини, якщо його достатньо на складі')
    ...
    def test_add_non_available_amount(self):
        ...
        self.assertEqual(self.cart.contains_product(self.product), False,
                         msg='Продукт не повинен бути доданий до корзини через нестачу кількості')
...
class TestEShop(unittest.TestCase):
    ...
    def test_product_is_available_true(self):
        ...
        self.assertTrue(self.product1.is_available(
            3), msg="Товар має бути доступним, оскільки запитується 3 одиниці, в наявності 5")
        self.assertTrue(self.product1.is_available(
            5), msg="Товар має бути доступним при запиті граничної кількості рівно 5 одиниць")
    ...
    def test_product_is_available_false(self):
        ...
        self.assertFalse(self.product1.is_available(
            6), msg="Метод is_available має повернути False, бо запитується 6 одиниць, в наявності лише 5")

    def test_product_buy_decreases_amount(self):
        # Чи коректно метод buy зменшує залишок товару.
        self.product1.buy(2)
        self.assertEqual(self.product1.available_amount, 3,
                         msg="Залишок товару після покупки 2 одиниць має дорівнювати 3, 5-2")
    ...
    def test_product_equality(self):
        ...
        self.assertEqual(self.product1, product_same_name,
                         msg="Товари з однаковим ім'ям Laptop мають вважатися рівними")
        self.assertNotEqual(self.product1, product_diff_name,
                            msg="Товари з різними іменами не мають вважатися рівними")
    ...
    def test_cart_add_existing_product_updates_amount(self):
        ...
        self.assertEqual(self.cart.products[self.product1], 3,
                         msg="Кількість товару в корзині має коректно підсумовуватися 2+1 = 3")
    ...
    def test_cart_submit_order_clears_cart_and_buys(self):
        ...
        self.assertEqual(self.product1.available_amount, 3,
                         msg="Кількість товару на складі не зменшилася після оформлення замовлення")
    ...
    def test_order_place_order_calls_submit(self):
        ...
        order.place_order(shipping_type="standard")
